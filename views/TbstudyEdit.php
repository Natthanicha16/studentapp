...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    ftbstudyedit = new ew.Form("ftbstudyedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = ftbstudyedit;

    // Add fields
    var fields = currentTable.fields;
    ftbstudyedit.addFields([
    ...
    // Form_CustomValidate
    ftbstudyedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ftbstudyedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    loadjs.done("ftbstudyedit");
});
</script>
<script>
...
<?php } ?>
<form name="ftbstudyedit" id="ftbstudyedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->study_id->Visible) { // study_id ?>
    <div id="r_study_id"<?= $Page->study_id->rowAttributes() ?>>
        <label id="elh_tbstudy_study_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->study_id->caption() ?><?= $Page->study_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->study_id->cellAttributes() ?>>
<span id="el_tbstudy_study_id">
<span<?= $Page->study_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->study_id->getDisplayValue($Page->study_id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="tbstudy" data-field="x_study_id" data-hidden="1" name="x_study_id" id="x_study_id" value="<?= HtmlEncode($Page->study_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
