...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fstudentsedit = new ew.Form("fstudentsedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fstudentsedit;

    // Add fields
    var fields = currentTable.fields;
    fstudentsedit.addFields([
    ...
    // Form_CustomValidate
    fstudentsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fstudentsedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fstudentsedit.lists.hobby = <?= $Page->hobby->toClientList($Page) ?>;
    loadjs.done("fstudentsedit");
});
</script>
<script>
...
<?php } ?>
<form name="fstudentsedit" id="fstudentsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" name="fa_x_student_images" id= "fa_x_student_images" value="<?= (Post("fa_x_student_images") == "0") ? "0" : "1" ?>">
...
<span id="el_students_hobby">
    <select
        ...
        data-select2-id="fstudentsedit_x_hobby"
        data-table="students"
        data-field="x_hobby"
...
<script>
loadjs.ready("fstudentsedit", function() {
    var options = { name: "x_hobby", selectId: "fstudentsedit_x_hobby" };
    if (fstudentsedit.lists.hobby.lookupOptions.length) {
        options.data = { id: "x_hobby", form: "fstudentsedit" };
    } else {
        options.ajax = { id: "x_hobby", form: "fstudentsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="students" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
...
<?php
    if (in_array("tbstudy", explode(",", $Page->getCurrentDetailTable())) && $tbstudy->DetailEdit) {
?>
...
?>
<?php
    if (in_array("tb_ta", explode(",", $Page->getCurrentDetailTable())) && $tb_ta->DetailEdit) {
?>
