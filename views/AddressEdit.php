...
<?php if ($Page->address_id->Visible) { // address_id ?>
    <div id="r_address_id"<?= $Page->address_id->rowAttributes() ?>>
        <label id="elh_address_address_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->address_id->caption() ?><?= $Page->address_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->address_id->getDisplayValue($Page->address_id->EditValue))) ?>"></span>
</span>
