 ...
 * Page class
 */
class AddressGrid extends Address
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AddressGrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->address_id->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_address_studentcode") && $CurrentForm->hasValue("o_address_studentcode") && $this->address_studentcode->CurrentValue != $this->address_studentcode->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_address_moo") && $CurrentForm->hasValue("o_address_moo") && $this->address_moo->CurrentValue != $this->address_moo->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_address_name") && $CurrentForm->hasValue("o_address_name") && $this->address_name->CurrentValue != $this->address_name->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_address_sub") && $CurrentForm->hasValue("o_address_sub") && $this->address_sub->CurrentValue != $this->address_sub->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_address_district") && $CurrentForm->hasValue("o_address_district") && $this->address_district->CurrentValue != $this->address_district->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_address_province") && $CurrentForm->hasValue("o_address_province") && $this->address_province->CurrentValue != $this->address_province->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_address_zip") && $CurrentForm->hasValue("o_address_zip") && $this->address_zip->CurrentValue != $this->address_zip->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->address_id->clearErrorMessage();
        $this->address_studentcode->clearErrorMessage();
        $this->address_moo->clearErrorMessage();
        $this->address_name->clearErrorMessage();
        $this->address_sub->clearErrorMessage();
        $this->address_district->clearErrorMessage();
        $this->address_province->clearErrorMessage();
        $this->address_zip->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->address_id->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_address_studentcode")) {
            $this->address_studentcode->setOldValue($CurrentForm->getValue("o_address_studentcode"));
        }
        ...
        if ($CurrentForm->hasValue("o_address_moo")) {
            $this->address_moo->setOldValue($CurrentForm->getValue("o_address_moo"));
        }
        ...
        if ($CurrentForm->hasValue("o_address_name")) {
            $this->address_name->setOldValue($CurrentForm->getValue("o_address_name"));
        }
        ...
        if ($CurrentForm->hasValue("o_address_sub")) {
            $this->address_sub->setOldValue($CurrentForm->getValue("o_address_sub"));
        }
        ...
        if ($CurrentForm->hasValue("o_address_district")) {
            $this->address_district->setOldValue($CurrentForm->getValue("o_address_district"));
        }
        ...
        if ($CurrentForm->hasValue("o_address_province")) {
            $this->address_province->setOldValue($CurrentForm->getValue("o_address_province"));
        }
        ...
        if ($CurrentForm->hasValue("o_address_zip")) {
            $this->address_zip->setOldValue($CurrentForm->getValue("o_address_zip"));
        }
    }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // address_studentcode
            if ($this->address_studentcode->getSessionValue() != "") {
                $this->address_studentcode->ReadOnly = true;
            }
            $this->address_studentcode->setDbValueDef($rsnew, $this->address_studentcode->CurrentValue, "", $this->address_studentcode->ReadOnly);

            // address_moo
            $this->address_moo->setDbValueDef($rsnew, $this->address_moo->CurrentValue, null, $this->address_moo->ReadOnly);

            // address_name
            $this->address_name->setDbValueDef($rsnew, $this->address_name->CurrentValue, "", $this->address_name->ReadOnly);

            // address_sub
            $this->address_sub->setDbValueDef($rsnew, $this->address_sub->CurrentValue, "", $this->address_sub->ReadOnly);

            // address_district
            $this->address_district->setDbValueDef($rsnew, $this->address_district->CurrentValue, "", $this->address_district->ReadOnly);

            // address_province
            $this->address_province->setDbValueDef($rsnew, $this->address_province->CurrentValue, "", $this->address_province->ReadOnly);

            // address_zip
            $this->address_zip->setDbValueDef($rsnew, $this->address_zip->CurrentValue, "", $this->address_zip->ReadOnly);

            // Check referential integrity for master table 'students'
            $detailKeys = [];
            $keyValue = $rsnew['address_studentcode'] ?? $rsold['address_studentcode'];
            $detailKeys['address_studentcode'] = $keyValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "students") {
            $this->address_studentcode->CurrentValue = $this->address_studentcode->getSessionValue();
        }
    ...
    protected function setupMasterParms()
    {
        ...
        if ($masterTblVar == "students") {
            $masterTbl = Container("students");
            $this->address_studentcode->Visible = false;
